...
def get_page_context(page) -> dict:
  """
  Collects essential page information for the LLM to understand the environment.

  Args:
      page: Playwright Page object representing the current browser tab.

  Returns:
      dict: A dictionary describing the page structure and interactive elements.
  """
...
def ask_llm_for_plan(goal, context) -> list[dict]:
  """
  Asks an LLM (OpenAI) to generate a JSON action plan for automating browser tasks.

  Args:
    goal (str): User's natural-language search goal.
    context (dict): Page context including elements and structure.

  Returns:
    list[dict]: A list of action dictionaries specifying automation steps.
  """
...
def run_llm_model(goal: str):
  """
  Executes a browser automation workflow planned by an LLM.

  Navigates to Amazon, requests a JSON action plan from the LLM, and executes each step sequentially.
  Displays product results after completion.

  Args:
      goal (str): The natural-language goal describing what to search for (e.g., "find the best-rated mouse").
